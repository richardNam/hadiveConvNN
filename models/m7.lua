-- model 37, added two more layers of Convo, with bigger kernel
...
model:add(nn.ReLU())
model:add(nn.SpatialConvolution(32,64, 3,3, 1,1, 0,0)):add(nn.Dropout(0.4))
model:add(nn.ReLU())
model:add(nn.SpatialMaxPooling(2,2,1,1))

model:add(nn.SpatialConvolution(64,128, 3,3, 1,1, 0,0))
model:add(nn.ReLU())
model:add(nn.SpatialConvolution(128,128, 3,3, 1,1, 0,0)):add(nn.Dropout(0.4))
model:add(nn.ReLU())
model:add(nn.SpatialMaxPooling(2,2,1,1))

model:add(nn.SpatialConvolution(128,128, 5,5, 1,1, 0,0))
model:add(nn.ReLU())
model:add(nn.SpatialConvolution(128,128, 5,5, 1,1, 0,0)):add(nn.Dropout(0.4))
model:add(nn.ReLU())
...
classifer:add(nn.Linear(dim, 256)):add(nn.Dropout(0.1))
